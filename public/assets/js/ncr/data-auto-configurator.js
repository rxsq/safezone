...
// Gets code and validates it against business rules
...
// Function which generates 3 digit NCR code, e.g., 001, 002, 003
...
document.addEventListener('DOMContentLoaded', async function () {
    // Utility to check if we are in edit view
    function isEditView() {
        return window.location.pathname.includes('/edit-ncr');
    }
    ...
    // Only set revision, engineer, and purchasing dates in edit view
    if (isEditView()) {
    ...
    // Only prefill engineering and purchasing fields in edit view
    if (isEditView()) {
        if ((!document.getElementById('engineering').value) &&
            ((sessionStorage.getItem("userRole") === "Administrator") || (sessionStorage.getItem("userRole") === "Engineering"))) {
            document.getElementById('engineering').value = sessionStorage.getItem('userName');
            document.getElementById('engineer-date').value = new Date().toISOString().substring(0, 10);
        }

        if ((!document.getElementById('purchasing').value) &&
            ((sessionStorage.getItem("userRole") === "Administrator") || (sessionStorage.getItem("userRole") === "Purchasing"))) {
